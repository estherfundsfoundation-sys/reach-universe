...
-- Private media bucket and owner-only upload access.
insert into storage.buckets (id, name, public)
values ('reach-media', 'reach-media', false)
on conflict (id) do update set public = false;

drop policy if exists "reach members upload their own media files" on storage.objects;
create policy "reach members upload their own media files"
on storage.objects for insert to authenticated
with check (bucket_id = 'reach-media' and (storage.foldername(name))[1] = auth.uid()::text);

drop policy if exists "reach members view their own media files" on storage.objects;
create policy "reach members view their own media files"
on storage.objects for select to authenticated
using (bucket_id = 'reach-media' and (storage.foldername(name))[1] = auth.uid()::text);

-- Do NOT make this bucket public. Staff should review consent before any photo is shared.
